...
Implement the complete 13-state FSM with all valid transitions, retry budgets,
and the PAUSED escape state — with 100% branch coverage and zero external
dependencies.

## Description

The FSM is the heart of Loom. It must be completely isolated from GitHub,
MCP, and SQLite — a pure Go package that can be fully tested with table-driven
unit tests and a fake clock.

All 13 states and all transitions defined in `docs/loom/analysis.md § 5` must
